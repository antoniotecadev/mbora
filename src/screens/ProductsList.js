...
export default function ProductsList({ route, navigation }) {
  ...
  const renderItemProduct = useCallback(({ item: product }) => { 
    return <Product key={product.id} appearanceColor={color} produto={product} userName={user.userFirstName + ' ' + user.userLastName} userTelephone={user.userTelephone} accountAdmin={user.accountAdmin} userIMEI={user.userIMEI} onPress={()=> showProductDetails(product)}/>
  },[]);
